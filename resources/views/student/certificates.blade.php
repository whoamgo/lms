...
@section('breadcrumbs')
    <a href="{{ route('student.dashboard') }}">Home</a> / My Certificates
@endsection
...
<div class="course-tabs" style="margin-top: 24px;">
    <button class="course-tab {{ $tab === 'current' ? 'active' : '' }}" onclick="window.location.href='{{ route('student.certificates', ['tab' => 'current']) }}'">
        Current
    </button>
    <button class="course-tab {{ $tab === 'past' ? 'active' : '' }}" onclick="window.location.href='{{ route('student.certificates', ['tab' => 'past']) }}'">
        Past
    </button>
    <button class="course-tab {{ $tab === 'upcoming' ? 'active' : '' }}" onclick="window.location.href='{{ route('student.certificates', ['tab' => 'upcoming']) }}'">
...
<div style="margin-top: 32px;">
    ...
    <div style="margin-bottom: 24px;">
        <h2 style="font-size: 1.5rem; font-weight: 600; color: #343541; margin: 0 0 8px 0;">
            This Certificate was issued to {{ strtolower($certificate->student->name) }}
        </h2>
        ...
        <div class="card">
            ...
            <div style="margin-bottom: 24px;">
                <div style="margin-bottom: 16px;">
                    ...
                    <div style="font-size: 0.875rem; font-weight: 500; color: #343541;">{{ $certificate->student->name }}</div>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <div style="font-size: 0.875rem; font-weight: 500; color: #343541;">{{ $certificate->course->title ?? 'Course' }}</div>
                </div>
                
                <div style="margin-bottom: 16px;">
                    <div style="font-size: 0.75rem; color: #6b7280; margin-bottom: 4px;">Certificate ID</div>
                    <div style="font-size: 0.875rem; font-weight: 500; color: #343541;">{{ $certificate->certificate_number }}</div>
                </div>
                
                <div style="margin-bottom: 16px;">
                    <div style="font-size: 0.75rem; color: #6b7280; margin-bottom: 4px;">Issued On</div>
                    <div style="font-size: 0.875rem; font-weight: 500; color: #343541;">{{ $certificate->issued_at->format('l, M d, Y') }}</div>
                </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <a href="{{ route('student.certificates.download', \App\Helpers\EncryptionHelper::encryptIdForUrl($certificate->id)) }}" 
                ...
                <button onclick="shareOnLinkedIn('{{ route('student.certificates.download', \App\Helpers\EncryptionHelper::encryptIdForUrl($certificate->id)) }}', '{{ $certificate->course->title ?? 'Course' }}')" 
                        class="btn btn-purple" 
