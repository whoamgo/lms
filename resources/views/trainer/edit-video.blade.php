...
@section('breadcrumbs')
    <a href="{{ route('trainer.dashboard') }}">Home</a> / <a href="{{ route('trainer.videos') }}">Uploaded videos</a> / Edit Video
@endsection
...
<div class="card">
    ...
    <form id="uploadVideoForm" method="POST" action="{{ route('trainer.videos.update', $video->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div style="border: 1px solid #e5e5e6; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" id="videoTitle" required placeholder="Enter video title" value="{{ $video->title }}">
            </div>
            
            <div class="form-group">
                <label>Description</label>
                <textarea name="description" id="videoDescription" rows="4" placeholder="Add brief description of your video">{{ $video->description }}</textarea>
            </div>
        </div>
        ...
        <div style="border: 1px solid #e5e5e6; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
            ...
            <div style="display: flex; gap: 24px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="radio" name="visibility" value="private" {{ $video->status === 'inactive' ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                ...
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="radio" name="visibility" value="public" {{ $video->status === 'active' ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width: 18px; height: 18px; color: #6b7280;">
        ...
        <div style="border: 1px solid #e5e5e6; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <input type="date" name="scheduled_at" id="scheduleDate" style="padding-right: 40px;" placeholder="dd/mm/yyyy" value="{{ $video->scheduled_at ? $video->scheduled_at->format('Y-m-d') : '' }}">
                ...
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        ...
                        <input type="time" name="scheduled_time" id="scheduleTime" style="padding-left: 40px;" placeholder="--:--" value="{{ $video->scheduled_at ? $video->scheduled_at->format('H:i') : '' }}">
                    </div>
                </div>
            ...
            <div class="form-group" style="margin-top: 20px;">
                ...
                <input type="number" name="duration_minutes" id="duration" min="1" placeholder="e.g., 60" value="{{ $video->duration_minutes }}">
            </div>
        </div>
        ...
        <div style="border: 1px solid #e5e5e6; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
            ...
            <div class="form-group" style="margin-bottom: 24px;">
                ...
                <div id="thumbnailPreview" style="margin-top: 12px;">
                    @if($video->thumbnail)
                        <img src="{{ asset($video->thumbnail) }}" style="max-width: 300px; border-radius: 8px; margin-top: 12px; border: 1px solid #e5e5e6;">
                    @endif
                </div>
            </div>
...
const VIDEO_UPDATE_URL = '{{ route("trainer.videos.update", $video->id) }}';
</script>
<script src="{{ asset('js/trainer-upload-video.js') }}"></script>
<script>
// Update form action for edit
$(document).ready(function() {
    $('#uploadVideoForm').attr('action', VIDEO_UPDATE_URL);
});
</script>
@endpush
@endsection
