// Trainer Add Class Form - AJAX Upload
$(document).ready(function() {
    ...
    form.on('submit', function(e) {
        ...
        if (date && time) {
            // Format as Y-m-d H:i for Laravel
            formData.append('scheduled_at', date + ' ' + time);
        } else {
            showAlert('error', 'Schedule date and time are required.');
        ...
        if (duration) {
            formData.set('duration', parseInt(duration)); // Already in minutes
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                showAlert('success', response.message || 'Class created successfully!');
                setTimeout(function() {
                    const redirectUrl = typeof LIVE_CLASSES_INDEX_URL !== 'undefined' ? LIVE_CLASSES_INDEX_URL : '/trainer/live-classes';
    ...
    function validateForm() {
        const title = $('#classTitle').val().trim();
